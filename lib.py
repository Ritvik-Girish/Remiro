...
import sqlite3
image1='D:\Vikas\Vikas 12 STD\School\COMP\COMP Project\Pic7.jfif'
image2='D:\Vikas\Vikas 12 STD\School\COMP\COMP Project\Pic6.jfif'
image3='D:\Vikas\Vikas 12 STD\School\COMP\COMP Project\Pic8.jfif'
class menu:
    ...
    def __init__(self):
        ...
        conn.execute('''create table if not exists book_info (ID VARCHAR PRIMARY KEY NOT NULL, TITLE VARTEXT NOT NULL, AUTHOR VARTEXT NOT NULL, GENRE VARTEXT NOT NULL, COPIES VARINT NOT NULL, LOCATION VARCHAR NOT NULL);''')
        conn.commit()
        conn.execute('''create table if not exists book_issued(BOOK_ID VARCHAR NOT NULL, STUDENT_ID VARCHAR NOT NULL, ISSUE_DATE DATE NOT NULL, RETURN_DATE DATE NOT NULL, PRIMARY KEY (BOOK_ID,STUDENT_ID));''')
        conn.commit()
        conn.close()
        self.a=self.canvases(image1)
        l1=Button(self.a,text='NEW BOOKING',font='Papyrus 22 bold',fg='Yellow',bg='Black',width=19,padx=10,borderwidth=0,command=self.book).place(x=100,y=500)
        l2=Button(self.a,text='YOUR ACCOUNT',font='Papyrus 22 bold',fg='Yellow',bg='Black',width=19,padx=10,borderwidth=0,command=self.student).place(x=800,y=500)
        self.root.mainloop()
    ...
    def book(self):
        ...
        l1=Button(self.a,text='ADD VENUES',font='Papyrus 22 bold',fg='Orange',bg='Black',width=15,padx=10,command=self.addbook).place(x=12,y=100)
        l2=Button(self.a,text='SEARCH VENUES',font='Papyrus 22 bold',fg='Orange',bg='Black',width=15,padx=10,command=self.search).place(x=12,y=200)

        l4=Button(self.a,text='BOOK N PAY',font='Papyrus 22 bold',fg='Orange',bg='Black',width=15,padx=10,command=self.all).place(x=12,y=300)
        l4=Button(self.a,text='<- HOME',font='Papyrus 22 bold',fg='Orange',bg='Black',width=15,padx=10,command=self.mainmenu).place(x=12,y=500)
    ...
    def addbook(self):
        ...
        l1=Label(self.f1,text='Venue Name : ',font='Papyrus 10 bold',fg='Orange',bg='Black',pady=1).place(x=50,y=50)
        e1=Entry(self.f1,width=45,bg='orange',fg='black',textvariable=self.aid).place(x=150,y=50)
        l2=Label(self.f1,text='Manager Name : ',font='Papyrus 8 bold',fg='Orange',bg='Black',pady=1).place(x=50,y=100)
        e2=Entry(self.f1,width=45,bg='orange',fg='black',textvariable=self.aname).place(x=150,y=100)
        l3=Label(self.f1,text='Contact Number : ',font='Papyrus 8 bold',fg='orange',bg='Black',pady=1).place(x=50,y=150)
        e3=Entry(self.f1,width=45,bg='orange',fg='black',textvariable=self.aauthor).place(x=150,y=150)
        l4=Label(self.f1,text='Venue Address : ',font='Papyrus 8 bold',fg='orange',bg='Black',pady=1).place(x=50,y=200)
    ...
    def adddata(self):
        a=self.avenuename.get()
        b=self.amanagername.get()
        c=self.acontactnumber.get()
        d=self.aaddress.get()
        conn=sqlite3.connect('test.db')
        try:
            if (a and b and c and d  and f)=="":
                ...
                values (?,?,?,?)",(a.capitalize(),b.capitalize(),c.capitalize(),d.capitalize(),));
                conn.commit()
                messagebox.showinfo("Success","Venue added successfully")
        except sqlite3.IntegrityError:
            messagebox.showinfo("Error","Venue is already present.")
    ...
    def search(self):
        ...
        self.f1.place(x=500,y=100)
        l1=Label(self.f1,text='Capacity of Guests: ',font=('Papyrus 10 bold'),bd=2, fg='orange',bg='black').place(x=20,y=40)
    ...
    def serch1(self):
        k=self.sid.get()
        if k!="":
            self.list4=("Venue Name","Manager Name","Contact Number","Venue Address")
            self.trees=self.create_tree(self.f1,self.list4)
            self.trees.place(x=25,y=150)
            conn=sqlite3.connect('test.db')

            c=conn.execute("select * from book_info where Capacity of Guests=?",(k.capitalize(),))
            a=c.fetchall()
            if len(a)!=0:
                for row in a:
                ...
                self.cm.config(values =('Delete Venue'))
    ...
    def student(self):
        self.a.destroy()
        self.a=self.canvases(image2)
        l1=Button(self.a,text='YOUR DETAILS',font='Papyrus 22 bold',fg='Orange',bg='Black',width=15,padx=10,command=self.issue).place(x=12,y=100)
        l2=Button(self.a,text='YOUR BOOOKINGS',font='Papyrus 22 bold',fg='Orange',bg='Black',width=15,padx=10,command=self.returnn).place(x=12,y=200)
        l3=Button(self.a,text='CUSTOMER SERVICE',font='Papyrus 22 bold',fg='Orange',bg='Black',width=15,padx=10,command=self.activity).place(x=12,y=300)
        l4=Button(self.a,text='< - HOME',font='Papyrus 22 bold',fg='Orange',bg='Black',width=15,padx=10,command=self.mainmenu).place(x=12,y=600)
    ...
    def issue(self):
        ...
        self.f1=Frame(self.a,height=550,width=1000,bg='black')
        self.f1.place(x=500,y=100)
        l1=Label(self.f1,text='Your Name : ',font='papyrus 15 bold',bg='black',fg='orange').place(x=50,y=100)
        e1=Entry(self.f1,width=25,bd=4,bg='orange',textvariable=self.aidd).place(x=180,y=100)
        l2=Label(self.f1,text='User Name : ',font='papyrus 15 bold',bg='black',fg='orange').place(x=50,y=150)
        e2=Entry(self.f1,width=25,bd=4,bg='orange',textvariable=self.astudentt).place(x=180,y=150)
        l3=Label(self.f1,text='Contact Details : ',font='papyrus 15 bold',bg='black',fg='orange').place(x=50,y=150)
        e3=Entry(self.f1,width=25,bd=4,bg='orange',textvariable=self.astudentt).place(x=180,y=150)
    ...
    def activity(self):
        ...
        self.list2=("VENUE NAME","MANAGER NAME","CONTACT NUMBER","VENUE ADDRESS")
...
def Database():
    ...
    if cursor.fetchone() is None:
        cursor.execute("INSERT INTO `login` (username, password) VALUES('123', '123')")
        conn.commit()
...
#==============================LABELS=========================================
lbl_title = Label(canvas, text = "USER   LOGIN", font=('Papyrus', 30,'bold', ),bg='black', fg='orange')
